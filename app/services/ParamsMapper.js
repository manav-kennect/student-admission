...
const mapper = function (obj, skeleton) {
    ...
    for (let key in skeleton) {
        ...
        if (!obj.hasOwnProperty(key) || obj[key] == '') {
            missingOne(key, { key, typeIs, typeExpected, reason: 'value not present, expecting: ' + typeExpected, code: 101 });
        } else if (typeExpected != typeIs) {
            missingOne(key, { key, typeIs, typeExpected, reason: 'value present, but not ' + typeExpected, code: 102 });
        } else {
